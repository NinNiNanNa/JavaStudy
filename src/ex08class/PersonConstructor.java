...
public class PersonConstructor {
	...
	/*
	디폴트 생성자
	: 해당 클래스에 생성자를 정의하지 않으면 아래와 같이 매개변수, 실행부가 없는 생성자가
	  컴파일러에 의해 자동으로 삽입된다.
...
//	public PersonConstructor() {}
	...
	public PersonConstructor(String name) {
		...
		클래스 자신, 즉 멤버를 가리킨다. 따라서 좌측항은 멤버변수, 우측항은 매개변수가 된다.
		*/
		this.name = name;
		age = 1;
		addr = "출처불명";
	...
	void initialize(String name, int age, String addr) {
		...
		멤버메서드는 개발자가 원할때 언제든 호출이 가능하므로 생성자와는 용도와 특성이 다르다.
		따라서 멤버메서드에서는 생성자를 호출할 수 없다.
		*/
//		this(name, age, "마상"); // => 에러발생
