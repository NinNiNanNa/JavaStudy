...
오버라이딩
: 메서드명, 매개변수, 반환형까지 모두 동일한 메서드를 상속관계에 있는 하위클래스에서 재정의 한것을 말한다.
...
class C extends B {
	/*
	rideMethod()는 오버라이딩 된 메서드로
	상위클래스에 정의된 멤버메서드와 반환형, 메서드명, 매개변수의 갯수 및 타입이 모두 동일하다.
	즉 똑같은 메서드이다.
	*/
	@Override
	public void rideMethod() {
	...
	*/
	...
	/*
	따라서 C클래스로 인스턴스를 생성하면 오버라이딩 된 메서드는 모두 가려지게되므로
	C클래스에 정의된 메서드 하나만 남게된다.
...
public class E07RideAndLoad01 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		즉 아래 부분은 모두 C클래스에 정의된 rideMethod가 호출된다.
		...
		*/
		...
		// B타입으로는 접근범위가 B클래스까지 제한된다.
		ref2.loadMethod();
		ref2.loadMethod(1);
//		ref2.loadMethod(1.1);
		
		// A타입으로는 접근번위가 A클래스로 제한된다.
		ref1.loadMethod();
//		ref1.loadMethod(1);
//		ref1.loadMethod(1.1);
