...
class Account {
	...
	*/
	...
	protected void depositMoney(int _money) {
		if(money<0) {
			System.out.println("마이너스 금액은 입급처리 불가합니다");
			return;
		}
...
class SavingAccount extends Account {
	...
	만약 아래와같이 호출하지 않으면 에러가 발생한다.
	*/
	public SavingAccount(int initVal) {
		/*
		만약 아래 문장을 삭제하면 즉시 에러가 발생한다.
		삭제하는 경우 super()라는 문장, 즉 매개변수가 없는 디폴트 생성자를 호출하는 문장이
		자동으로 추가되는데, 부모클래스에는 이런 형태의 생성자가 없으므로 아래와 같이 명시해야한다.
		*/
		super(initVal);
	}
	/*
