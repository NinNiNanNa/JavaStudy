...
//	public Computer() {}
	...
	외부 클래스 혹은 상속받은 클래스에서 접근이 불가능하므로,
...
class NotebookComputer extends Computer {
	...
	public void movingCal() {
		...
		if(battary<1) {
			...
			// 메서드에서 아래와 같이 return을 만나서 실행이 중지된다.
			return;
		}
		...
		상속을 받은 자식클래스에서도 접근할 수 없어 에러가 발생된다.
...
/*
태블릿 노트북
: 컴퓨터의 기본 연산기능과, 휴대이용 그리고 전용 펜으로 드로잉 할 수 있는 기능까지
  추가된 컴퓨터를 추상화한다.
*/
class TabletNotebook extends NotebookComputer {
	...
	public void write(String penInfo) {
		...
		A.compareTo(B)
		: 문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열이고 -1 혹은 1이 반환되면
		...
		*/
		...
		상속받은 자식클래스에서는 이와같이 부모의 멤버메서드를 호출하여
		자신의 기능처럼 사용할 수 있다.
		단 접근이 가능한 지정자로 선언되어야한다.
		super는 생략할 수 있다.
		*/
		super.movingCal();
