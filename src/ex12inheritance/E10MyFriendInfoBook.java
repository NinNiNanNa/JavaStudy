...
//		System.out.println("addFriend 호출됨");
	...
	4. 출력을 위한 멤버메서드는 상속관계에서 오버라이딩 처리되어 있으므로
	   참조변수의 영향을 받지않고 항상 자식클래스에 오버라이딩된 메서드가 호출된다.
	5. 즉 저장된 인스턴스는 Friend타입이지만 오버라이딩을 통해 별도의 형변환이 필요하지 않다.
...
//		System.out.println("showSimpleData 호출됨");
		...
		정보가 저장되는 배열은 Friend타입이므로 자식인스턴스를 저장하면 자동으로 형변환(업캐스팅)된다.
		이때 자식의 정보를 출력하는 메서드를 바로 호출할 수 없으므로
		아래와 같이 일일이 확인한 후 강제형변환(다운캐스팅) 해야한다.
		for(int i=0; i<numOfFriends; i++) {
...
//		System.out.println("deleteInfo 호출됨");
		...
		/*
		배열의 원소 중 삭제된 원소의 인덱스값을 저장하기 위한 변수로 삭제한 후 빈자를 채워넣을때 사용한다.
		...
		*/
		...
		if(deleteIndex==-1) {
			...
			만약 이부분이 처리되지않는다면 차후
			검색이나 삭제시 NullPointerException이 발생할 수 있다.
			*/
