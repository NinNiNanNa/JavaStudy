...
- Array라는 이름처럼 '배열'의 특성을 가지고있어 index를 통한 저장 및 접근이 가능하다.
*/
...
//		list.add(5, "소방차ㅋㅋ"); // 에러발생됨
		...
		*/
		...
		방법1 : 일반 for문 사용. 이때는 인덱스를 통해 접근해야하므로 get()메서드를 사용한다.
		*/
		System.out.println("\n[일반for문 사용]");
		...
		*/
		...
		/*
		삽입하기(끼워넣기)
			add(삽입할인덱스, 인스턴스) : 해당 인덱스에 인스턴스를 삽입한다.
		...
		*/
		...
		/*
		포함여부확인
			contains(객체) : 특정 인스턴스가 저장되어있는지 확인할때 사용한다.
							존재한다면 true를 반환한다.
		*/
		...
		삭제방법1 : 인덱스로 삭제
			remove(인덱스) : 인덱스를 통해 삭제를 진행한 후 삭제가 완료되면
						   해당 인스턴스를 반환한다. 삭제 후에는 인덱스가 자동으로 부여된다.
						   삭제 성공시 해당 인스턴스를 반환하므로 삭제 후 즉기 확인할 수 있다.
		*/
