...
- main() 메서드가 실행되기전에 초기화되므로 main()메서드 실행시 별도의 선언없이 변수를 사용할 수 있다.
...
class MyStatic {
	/*
	멤버변수
	: 인스턴스형 멤버변수는 반드시 클래스의 인스턴스를 생성한 후 참조변수를 통해 접근해야한다.
	...
	*/
	...
	/*
	정적멤버변수
	: 객체를 생성할 필요없이 클래스명으로 바로 접근할 수 있다.
	...
	*/
	void instanceMethod() {
		System.out.println("instancsVar = "+ instancsVar);
	...
	/*
	정적멤버메서드
	: 정적메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
	  정적 멤버만 접근할 수 있다.
	  (※ main 메서드에서는 static으로 선언된 메서드만 호출할 수 있었음을 기억하자.)
	*/
	static void staticMethod() {
//		System.out.println("instancsVar = "+ instancsVar); // 에러발생
		System.out.println("staticVar = "+ staticVar);
//		instanceMethod(); // 에러발생
	}
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		일반적인 멤버변수에 접근하는 방법
		: MyStatic클래스의 인스턴스를 생성한 후 참조변수를 통해 접근한다.
		*/
		MyStatic myStatic = new MyStatic();
		myStatic.instancsVar = 100;
		System.out.println("myStatic.instancsVar = "+ myStatic.instancsVar);
		...
		*/
		ms1.instancsVar = 100;
		ms2.instancsVar = 1000;
		System.out.println("ms1.instancsVar = "+ ms1.instancsVar); // 100
		System.out.println("ms2.instancsVar = "+ ms2.instancsVar); // 1000
		...
		*/
		...
		결국 하나의 정적변수에 값을 입력한 결과가 되므로, 정적변수에 접근시에는 참조변수를 사용하지말고,
		클래스명을 사용하는 것이 좋다.
		...
		*/
		...
		인스턴스메서드는 인스턴스 참조변수를 통해서만 접근할 수 있다.
		*/
