...
class NormalThread extends Thread {
	...
	*/
	...
	/*
	Thread클래스의 생성자 호출시 전달하는 인수를 통해 해당 쓰레드의 이름을 지정할 수 있다.
...
class DaemonThread extends Thread {
	...
	*/
	...
	public void run() {
		while(true) {
			// getName() : 쓰레드명을 반환한다.
			System.out.printf("[쓰레드명:%s]Jazz가 흘려요~%n", getName());
			try {
				sleep(3000);
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		...
		/*
		쓰레드로 구현하지 않는 일반적인 클래스의 작업으로 순차적으로 진행되어 1번 인스턴스가 작업을
		마쳐야만 2번째가 실행된다.
		*/
...
//		yt1.setName("First 쓰레드");
		...
		*/
		...
		/*
		데몬쓰레드는 독립쓰레드를 먼저 생성한 후 setDaemon()을 통해 데몬쓰레드로 만들어준다.
		*/
