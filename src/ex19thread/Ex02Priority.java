	...
	public void run() {
		...
		*/
		for(int i=1; i<=1000; i++) {
			...
			sleep()을 호출하면 실행중 잠깐식 블럭상태로 전환되어 
			우선순위가 낮은 쓰레드도 실행될 수 잇는 기회가 생기게된다.
			*/
			try {
				sleep(1);
			}
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		1단계실행
		: 우선순위를 부여하지 않은 상태로 쓰레드 인스턴스를 생성하면 동일한 우선순위 5를 부여받게된다.
		  이때는 CPU가 알아서 작업을 분배한다.
		*/
//		MessageSendingThread tr1 = new MessageSendingThread("첫번쩨");
//		MessageSendingThread tr2 = new MessageSendingThread("두번쩨");
//		MessageSendingThread tr3 = new MessageSendingThread("세번쩨");
