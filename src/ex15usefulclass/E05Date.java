...
public class E05Date {
	...
	*/
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		getTime() : 서버의 표준시인 1970-01-01 부터 현재까지의 시간을 초 단위로 반환해준다.
					이를 타임스템프라고 한다.
		*/
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이 동일하기 때문에 구분을 위해
		월은 대문자, 분은 소문자를 사용한다.
		*/
		...
		*/
		...
		여기서는 두가지 서식의 날짜를 파싱하여 구한 날짜객체를 통해 계산한다.
		*/
		// 오늘날짜 : 0000-00-00 형식
...
//		System.out.println(today+ " "+future);
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			*/
			Date formatToday = new SimpleDateFormat("yyy-MM-dd").parse(today);
			Date formatFuture = new SimpleDateFormat("yyy/MM/dd").parse(future);
			System.out.println(formatToday+" <> "+formatFuture);
			
			/*
			getTime()으로 얻어오는 시간은 13자리의 정수값인데 10자리까지는 순수한 초(Sec)이고,
			11~13자리는 밀리세컨즈를 의미한다.
			즉 100으로 나눠서 뒤의 3자리를 제거하면 순수한 추가 반환된다.
